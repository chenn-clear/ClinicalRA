    ...
    def load_annotations(self):
        ...
        with open(self.ann_file, 'r') as f:
            for i, line in enumerate(f):
                ...
                # Filename and label
                filename = items[0]
                gt_label = int(items[1])
                
                # Extract attributes (assumed to be columns 2 to 9)
                attributes = list(map(float, items[2:10]))

                # Normalize each attribute to [0,1]
                # normalized_attributes = []
                # for idx, value in enumerate(attributes):
                #     if idx == 2:  # Third feature range [1,6]
                #         normalized_value = (value - 1) / 5.0
                #     else:          # Other feature range [1,5]
                #         normalized_value = (value - 1) / 4.0
                #     normalized_attributes.append(normalized_value)

                # normalized_attributes = np.array(normalized_attributes, dtype=float)

                # # L2 normalize the entire feature vector
                # l2_value = norm(normalized_attributes)
                # if l2_value != 0:
                #     normalized_attributes = normalized_attributes / l2_value

                # # Assign the final normalized result back to attributes
                ...
                info = {
                    ...
                    'img_text': '',
                    'attributes': attributes  # Add attributes field
                }
                
                # If feature_file exists, add feature information
                if self.feature_file is not None:
